...
class ActorCritic(nn.Module):
    def __init__(self, num_inputs, num_outputs, hidden_size, action_std):
        ...
        self.log_action_std = nn.Parameter(torch.tensor([action_std] * num_outputs))
...
class PPO:
    def __init__(self, num_inputs, num_outputs, lr, action_std, agent_ids, hidden_size=128, ppo_epochs=8, mini_batch_size=2048, inference=False):
    ...
    def select_action(self, agent_id, state):
        with torch.no_grad():
            ...
            if self.inference:
                self.action[agent_id] = self.dist[agent_id].mean
            else:
    ...
    def load(self, filename, directory):
        ...
        state_dict['log_action_std'] = torch.ones(2).to(device)
